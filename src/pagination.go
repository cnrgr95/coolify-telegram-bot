...
func Paginate(totalItems int, currentPage int, pageSize int, callbackPrefix string) (startIndex, endIndex int, buttons []PageButton) {
	...
	if currentPage > 1 {
		buttons = append(buttons, PageButton{Text: "< Önceki", Data: fmt.Sprintf("%s%d", callbackPrefix, currentPage-1)})
	}
	...
	if currentPage < totalPages {
		buttons = append(buttons, PageButton{Text: "Sonraki >", Data: fmt.Sprintf("%s%d", callbackPrefix, currentPage+1)})
	}
